...
class LLMChatDialog(QDialog):
    ...
    def __init__(self, frame_rgb: np.ndarray, llm_metadata_path, llm_precision, parent=None):
        ...
        # 修复字体警告：Mac自动用系统字体，Windows用Segoe UI
        font = QFont()
    ...
    def _start_process(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            self.process.setProgram(sys.executable)
            self.process.setArguments(["-u", script_path, "--cfg", self.llm_metadata_path, "--image", self.temp_img_path, "--precision", self.llm_precision])
    ...
    def _on_process_finished(self, exitCode, exitStatus):
        if exitCode != 0 and not self.model_ready:
            ...
            if exitCode == 137 or exitCode == -9:
                err_msg += "<span style='color:gray;'>Reason: Out of Memory. Your Mac killed the process because it ran out of RAM.</span>"
            self.txt_history.append(err_msg)

    def _on_process_error(self, error):
        self.txt_history.append(f"<br><b style='color:#FA5252;'>[System] Failed to start process: {error}</b><br>")

    def _handle_stdout(self):
        # 1. 把通道里现有的所有字节全部读出来，拼接到我们的缓冲区里
        raw_bytes = self.process.readAllStandardOutput().data()
        self._stdout_buffer += raw_bytes.decode("utf-8", errors="replace")

        # 2. 只要缓冲区里有换行符，说明凑够了一整行完整的 JSON
        while "\n" in self._stdout_buffer:
            ...
            try:
                ...
                # 打印出不是 JSON 的系统调试信息或报错
                print(f"[Mac Debug Log]: {line}")
