...
# Disable Hugging Face / tqdm progress bars
os.environ["HF_HUB_DISABLE_PROGRESS_BARS"] = "1"
os.environ["TQDM_DISABLE"] = "1"


# ============================================================
# PyInstaller compatibility (macOS Apple Silicon)
# ============================================================
...
# Fixes some download failures on macOS
try:
...
# ============================================================
# Logging / stdout / stderr safety for packaged exe
# ============================================================

# Logging to stdout never happens in exe, but some libraries
# (e.g. ultralytics) may still try and can crash if encoding is invalid.
    ...
    def _on_model_changed(self):
        ...
        if "Refine Cell Subpopulations" in self.additional_config_inputs:
            ...
            def update_refine_ui():
                ...
                if isinstance(cmb_refine, QCheckBox):

                    is_active = cmb_refine.isChecked()
                    is_km = is_active  
                    is_th = is_active  
                else:
                ...
                if row_refine_preview: row_refine_preview.setVisible(is_km or is_th)
            ...
            if isinstance(cmb_refine, QCheckBox):
                cmb_refine.stateChanged.connect(lambda _: update_refine_ui())
            else:
